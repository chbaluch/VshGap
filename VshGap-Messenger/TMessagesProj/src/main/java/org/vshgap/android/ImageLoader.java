...
import org.vshgap.messenger.DispatchQueue;
        ...
        FileLoader.getInstance().setDelegate(new FileLoader.FileLoaderDelegate() {
            ...
            public void fileDidLoaded(final String location, final File finalFile, final int type) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        if (location != null) {
                            if (MediaController.getInstance().canSaveToGallery() && vshgapPath != null && finalFile != null && finalFile.exists() && (location.endsWith(".mp4") || location.endsWith(".jpg"))) {
                                if (finalFile.toString().startsWith(vshgapPath.toString())) {
    ...
    private HashMap<Integer, File> createMediaPaths() {
        ...
        try {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                vshgapPath = new File(Environment.getExternalStorageDirectory(), "VshGap");
                vshgapPath.mkdirs();
                ...
                try {
                    for (int a = 0; a < 5; a++) {
                        ...
                        File dstFile = new File(vshgapPath, "temp.file");
                ...
                if (canRename) {
                    if (vshgapPath.isDirectory()) {
                        try {
                            File imagePath = new File(vshgapPath, "VshGap Images");
                            imagePath.mkdir();
                        ...
                        try {
                            File videoPath = new File(vshgapPath, "VshGap Video");
                            videoPath.mkdir();
                        ...
                        try {
                            File audioPath = new File(vshgapPath, "VshGap Audio");
                            audioPath.mkdir();
                        ...
                        try {
                            File documentPath = new File(vshgapPath, "VshGap Documents");
                            documentPath.mkdir();
                        ...
                        // Themes
                        try {
                            File themesPath = new File(vshgapPath, "Themes");
                            themesPath.mkdir();
